        ...
        <CardContent className="pt-6 pb-6 relative">
          <div className="flex flex-col md:flex-row justify-between gap-6">
            ...
            <div className="flex items-center gap-3">
              {totalWorkflows > 0 && (
                <Badge variant="outline" className={`${progressStatus.textColor} border-current flex items-center gap-1.5 px-3 py-1.5 font-medium`}>
                  ...
                  {progressStatus.next && (
                    <div className="w-full bg-muted/30 rounded-full h-2.5 overflow-hidden">
                      <div 
                        ...
                        style={{ width: `${Math.min(100, (totalWorkflows / progressStatus.next) * 100)}%` }}
                      ></div>
                    </div>
                  )}
                  ...
                  <div className="pt-2">
                    ...
                    <ul className="space-y-2 text-sm">
                      <li className="flex items-start gap-2">
                        <div className={`${progressStatus.color} rounded-full p-0.5 mt-0.5`}>
                      ...
                      {(progressStatus.level === "Explorer" || progressStatus.level === "Expert") && (
                        <li className="flex items-start gap-2">
                          <div className={`${progressStatus.color} rounded-full p-0.5 mt-0.5`}>
                            <svg className="h-3 w-3 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M5 13l4 4L19 7" />
            ...
            <CardContent className="flex-grow py-6">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="space-y-3 group">
                  ...
                  <h3 className="font-medium group-hover:text-primary transition-colors">Share Your Knowledge</h3>
                  <p className="text-sm text-muted-foreground">
                ...
                <div className="space-y-3 group">
                  ...
                  <h3 className="font-medium group-hover:text-primary transition-colors">Build Authority</h3>
                  <p className="text-sm text-muted-foreground">
                ...
                <div className="space-y-3 group">
                  ...
                  <h3 className="font-medium group-hover:text-primary transition-colors">Future Income</h3>
                  <p className="text-sm text-muted-foreground">
              ...
              <div className="mt-8 flex justify-center">
                <Button asChild className="w-full max-w-xs py-6 text-lg gap-2 bg-primary hover:bg-primary/90 shadow-md hover:shadow-lg transition-all duration-200">
            ...
            <CardFooter className="bg-amber-50 dark:bg-amber-950/20 border-t border-amber-200/50 dark:border-amber-800/20 text-amber-800 dark:text-amber-300 p-3">
              <div className="flex items-center gap-2 w-full text-sm">
                ...
                <p>
                  ...
                  Workflow Marketplace - Sell your automation solutions and earn money!
                </p>
              </div>
            </CardFooter>
          </Card>
      ...
      <Card className="shadow-md overflow-hidden">
          ...
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
            ...
            <div className="flex items-center gap-3">
              <div className="flex flex-col items-center justify-center bg-primary/5 rounded-lg px-4 py-2 border border-primary">
                ...
                <span className="text-xs text-muted-foreground mt-1">Total Workflows</span>
              </div>
              {categoriesUsed.length > 0 && (
                <Badge variant="outline" className="font-medium px-3 border border-primary">
        ...
        <CardContent className="p-6">
          {categoriesUsed.length > 0 ? (
            <>
              ...
              <div className="mb-6">
                <h3 className="text-sm font-medium text-muted-foreground mb-4 flex items-center gap-2">
                  ...
                  <Badge variant="secondary" className="font-normal text-xs">Most Used</Badge>
                </h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                  {topCategories.map((category, index) => {
                    const percentage = Math.round((category.count / totalWorkflows) * 100);
                    const colors = [
                    ...
                    return (
                      <div 
                        key={category.name}
                        className={`${colors[index].split(' ')[0]} ${colors[index].split(' ')[1]} rounded-lg p-4 border-l-4 ${colors[index].split(' ')[2]} shadow-sm hover:shadow-md transition-all duration-300 group`}
                      >
              ...
              <div>
                <h3 className="text-sm font-medium text-muted-foreground mb-4 uppercase tracking-wider">ALL CATEGORIES</h3>
                <div className="space-y-4">
                    ...
                    return (
                      <div key={category.name} className="space-y-2 group">
                        <div className="flex justify-between items-center">
                          <Link href={`/?category=${category.name}`} className="flex items-center gap-2 hover:text-primary transition-colors">
                            <span 
            ...
            <div className="text-center py-8">
              ...
              <Button className="mx-auto bg-primary hover:bg-primary/90" asChild>
                <Link href="/dashboard/wf/create" className="flex items-center gap-2">
                  <Plus className="h-5 w-5" />
        ...
        <CardFooter className="bg-muted/10 border-t p-4">
          <Button variant="ghost" className="ml-auto" asChild>
            <Link href="/dashboard/wf/create" className="flex items-center gap-1 text-primary">
