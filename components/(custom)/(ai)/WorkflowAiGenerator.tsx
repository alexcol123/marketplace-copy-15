...
export default function WorkflowAIGenerator() {
  ...
  useEffect(() => {
    ...
    const descriptionTextarea = document.querySelector('textarea[name="content"]');
    const jsonTextarea = document.querySelector('textarea[name="workFlowJson"]');
    const stepsContainer = document.querySelector('[data-steps-container]');
    const categorySelect = document.querySelector(`select[name="${CATEGORY_FIELD_NAME}"]`);
    
    // Set the refs
    if (titleInput) titleInputRef.current = titleInput as HTMLInputElement;
    if (descriptionTextarea) descriptionTextareaRef.current = descriptionTextarea as HTMLTextAreaElement;
    if (jsonTextarea) jsonTextareaRef.current = jsonTextarea as HTMLTextAreaElement;
    if (stepsContainer) stepsContainerRef.current = stepsContainer as HTMLDivElement;
    if (categorySelect) categorySelectRef.current = categorySelect as HTMLSelectElement;
  }, []);
  ...
  const generateSteps = async () => {
    ...
    try {
      ...
      const data = await response.json();
      console.log("Steps data received:", data); // Debug log
      ...
      if (data.success && data.steps && Array.isArray(data.steps)) {
        ...
        const addStepsButton = document.querySelector('[data-add-steps-button]');
        if (addStepsButton) {
          // Check if steps section is already expanded
          const stepsContainer = document.querySelector('[data-steps-container]');
          const isExpanded = stepsContainer?.querySelector('textarea[data-step-index]');
        ...
        if (addStepButton) {
          ...
          const existingSteps = document.querySelectorAll('textarea[data-step-index]');
          const neededClicks = Math.max(0, data.steps.length - existingSteps.length);
        ...
        data.steps.forEach((step: string, index: number) => {
          setTimeout(() => {
            const stepTextarea = document.querySelector(`textarea[data-step-index="${index}"]`) as HTMLTextAreaElement;
            if (stepTextarea) {
              stepTextarea.value = step;
              stepTextarea.dispatchEvent(new Event('input', { bubbles: true }));
              console.log(`Updated step ${index}`);
            } else {
  ...
  const generateCategory = async () => {
    ...
    try {
      ...
      if (data.success && data.category) {
        ...
        const isValidCategory = technologyCategories.some(cat => cat.label === data.category);
        ...
        const hiddenInput = document.querySelector(`input[name="${CATEGORY_FIELD_NAME}"][type="hidden"]`);
        if (hiddenInput) {
          ...
          console.log("Updated hidden input value to:", category);
        }
        
        // Also update the visible trigger text
        const triggerElement = document.querySelector(`#${CATEGORY_FIELD_NAME} .select-value, #${CATEGORY_FIELD_NAME} [class*="SelectValue"]`);
        if (triggerElement) {
          // Try to update the text content directly
          triggerElement.textContent = category;
          console.log("Updated trigger text to:", category);
        }
        
        // Force a React state update by creating a custom event
        document.dispatchEvent(new CustomEvent('shadcn-select-update', { 
  ...
  const generateAll = async () => {
    ...
    try {
      ...
      const data = await response.json();
      console.log("All content data received:", data); // Debug log
      ...
      if (data.success) {
        ...
        if (data.category) {
          ...
          console.log("Setting category from generateAll to:", category);
          ...
          console.log("Dispatched shadcn-select-update event with category:", category);
        }
        ...
        if (data.steps && Array.isArray(data.steps) && data.steps.length > 0) {
          ...
          if (addStepButton) {
            ...
            console.log(`Need to add ${neededClicks} more steps`);
          ...
          setTimeout(() => {
            data.steps.forEach((step: string, index: number) => {
              ...
              if (stepTextarea) {
                stepTextarea.value = step;
                stepTextarea.dispatchEvent(new Event('input', { bubbles: true }));
                console.log(`Updated step ${index} with: ${step.substring(0, 30)}...`);
              } else {
  ...
  return (
    <div className="flex flex-col gap-2">
      <div className="text-sm font-medium mb-1">AI-Powered Content Generation</div>
      <div className="flex flex-wrap gap-2">
        ...
        <Button
          type="button"
          onClick={generateAll}
          disabled={isGenerating.title || isGenerating.description || isGenerating.steps || isGenerating.category || isGenerating.all}
          variant="default"
          size="sm"
          className="gap-2"
        >
      ...
      <p className="text-xs text-muted-foreground mt-1">
        Automatically generate content from your workflow JSON. Requires valid JSON structure.
      </p>
    </div>
  );
}